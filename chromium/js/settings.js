...
async function renderDefaultMode() {
    // Load the saved default filtering mode from local storage
    const savedMode = await localRead('defaultFilteringMode');
    const defaultLevel = savedMode !== null ? parseInt(savedMode, 10) : cachedRulesetData.defaultFilteringMode;
...
async function onFilteringModeChange(ev) {
    ...
    switch (newLevel) {
    case 0: { // Revoke broad permissions
        await browser.permissions.remove({
            origins: ['<all_urls>']
        });
        cachedRulesetData.defaultFilteringMode = 0;
        break;
    }
    case 1: { // Request broad permissions
    ...
    // Save the new default filtering mode to local storage
    localWrite('defaultFilteringMode', newLevel);
    renderFilterLists();
    renderWidgets();
}
